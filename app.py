...
def add_recipe():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if image.filename: # if image was uploaded
            ...
            filename = secure_filename(str(uuid.uuid1()) + file_extension) # get a unique filename
            destination = "".join([target, filename]) #create path to save the file
...
def show_recipe(recipe_id):
    ...
    recipes = mongo.db.recipes
    if ObjectId.is_valid(recipe_id) and recipes.count_documents({'_id': ObjectId(recipe_id), 'owner': current_user.email}) > 0: #the validity check is needed in case user tries to type the url themselves
